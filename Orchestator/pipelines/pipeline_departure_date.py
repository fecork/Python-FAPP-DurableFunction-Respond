...
def pipeline(context: df.DurableOrchestrationContext, parameters_dict: dict):
    ...
    question_fare_rules = parameters["question_fare_rules_departure_date"]
    structure_fare_rules = parameters["structure_fare_rules"]
    structure_questions = parameters["structure_fare_rules_departure_date"]

    parameters_dict["task"] = "DEPARTUREDATE"
    ...
    quiz_text_and_question = (
        'SURCHARGES'
        + "\n"*2
        + gpt_paragraph_text['DEPARTUREDATE']
        + "\n" * 2
        + question_fare_rules
        + "\n" * 2
